...
def readXml(filename):
    ...
    tree = ET1.parse('C:\\StackOverflow\\Python\\snippets\\post' + filename + '_snippets.xml')
    root = tree.getroot();

    unparseroot = ET2.Element("DATA");
    parseroot = ET3.Element("DATA");
    
    with open('C:\\StackOverflow\\Python\\parseError27\\post' + filename + '_error.txt', 'w') as f:       
        for row in root:    
        ...
        unparsetree.write('C:\\StackOverflow\\Python\\unparsablePython27\\post' + filename + '_unparsable.xml');
        parsetree = ET3.ElementTree(parseroot)
        parsetree.write('C:\\StackOverflow\\Python\\parsablePython27\\post' + filename + '_parsable.xml');
